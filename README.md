...
This helm chart installs the [Wishlist web application](https://github.com/cmintey/wishlist) (under [MIT license](https://github.com/cmintey/wishlist?tab=MIT-1-ov-file)) written by [cmintey](https://github.com/cmintey/).  This is an **unofficial chart** utilising the official Docker image. 
...
values.yaml for Traefik (see also [example wishlist overrides](#example-wishlist-overrides) from the original readme)
```yaml
ingress:
  annotations:
...
> Note that in the example above, `websecure` and `internalacme` depend on my Traefik config. YMMV. [RTFM](https://doc.traefik.io/traefik/). All that good stuff. 
